...
from PIL import Image, ImageOps, ImageSequence, ImageFile, ImageEnhance, ImageFilter
					...
					for sched in SCHEDULERS_ANY:
						if sampler_full.lower().endswith(' ' + sched.lower()):
							...
							if sched_start >= 0:
								...
								gen_data["sampler"] = sampler_full[:sched_start].strip()
...
class RvImage_LoadImagePathWithMetadata:
	@classmethod
	def INPUT_TYPES(s):
		return {
				"required": 
	...
	RETURN_TYPES = ("IMAGE", "MASK", "STRING", "STRING", "INT", "INT", "INT", "STRING", "STRING", "FLOAT", "INT", "STRING")
	RETURN_NAMES = ("image", "mask", "prompt", "negative", "width", "height", "steps", "sampler", "scheduler", "cfg_scale", "seed", "version")
	FUNCTION = "load_image"
...
NODE_NAME = 'Load Image from Path (Metadata) [RvTools-X]'
NODE_DESC = 'Load Image from Path (Metadata)'

NODE_CLASS_MAPPINGS = {
   NODE_NAME: RvImage_LoadImagePathWithMetadata
}
