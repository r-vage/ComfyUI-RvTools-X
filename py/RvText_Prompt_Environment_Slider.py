...
class RvText_Prompt_Environment_Slider:
        ...
        return {
            "required": {
                **cls.get_input_types_from_keys(cls.CATEGORY_KEYS),
                "seed": ("INT", {"default": 0, "min": -1125899906842624, "max": 1125899906842624, "tooltip": "Random seed for environment selection."}),
            }
        }
    ...
    def get_input_types_from_keys(keys):
        ...
        for key in keys:
            input_types[key] = (
                tuple(RvText_Prompt_Environment_Slider.get_options_keys(key)),
                {
                    "default": "None",
                    "tooltip": f"Select {key} environment option."
                }
            )
...
NODE_NAME = 'Prompt Environment Slider [RvTools-X]'
