...
class RvText_Prompt_Subject_Slider:
    ...
    CATEGORY_KEYS = [
        'Character', 'Subject Type', 'Action', 'Action+', 'Action++', 'Positioning', 'Hair', 'Rare Hairstyle',
        'Rare Hairstyle Man', 'Rare Hair Colors', 'Head Accessories', 'Face', 'Ears', 'Neck', 'Skin', 'Clothing',
        'Upper Body Decoration', 'Lower Body Decoration', 'Full body decoration', 'Shoes and socks', 'Accessories'
    ]
        ...
        return {
            "required": {
                **cls.get_input_types_from_keys(cls.CATEGORY_KEYS),
                "seed": ("INT", {"default": 0, "min": -1125899906842624, "max": 1125899906842624, "tooltip": "Random seed for subject selection."}),
            }
        }
    ...
    def get_input_types_from_keys(keys):
        ...
        for key in keys:
            input_types[key] = (
                tuple(RvText_Prompt_Subject_Slider.get_options_keys(key)),
                {
                    "default": "None",
                    "tooltip": f"Select {key} subject option."
                }
            )
...
NODE_NAME = 'Prompt Subject Slider [RvTools-X]'
