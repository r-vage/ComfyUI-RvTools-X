...
class RvText_RandomPrompt:
    ...
    def INPUT_TYPES(cls):
        required = {}
        # Only load files matching 0_0Name.txt pattern in json/settings
        if not os.path.isdir(SETTINGS_PATH):
            os.makedirs(SETTINGS_PATH, exist_ok=True)
        for fname in os.listdir(SETTINGS_PATH):
            if fname.lower().endswith('.txt') and re.match(r'^\d+_\d+\w+\.txt$', fname):
                base = os.path.splitext(fname)[0]
                display = re.sub(r'^[0-9_]+', '', base)
                fpath = os.path.join(SETTINGS_PATH, fname)
                try:
...
NODE_NAME = 'Random Prompt by jice: Settings [RvTools-X]'
